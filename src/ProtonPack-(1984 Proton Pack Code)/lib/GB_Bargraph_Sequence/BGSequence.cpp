...
{
  if (hardwareType == BARGRAPH_COMMON_ANODE) {
    barGraphMap = barGraph_anode;
    _invertLogic = false; // Do not invert logic for anode
  } else {
    barGraphMap = barGraph_cathode;
    _invertLogic = true; // Invert logic for cathode
  }
...
void BGSequence::sequenceFire1(unsigned long currentMillis)
{
  if ((currentMillis - lastUpdateBG) > IntervalBG)
  {
    ...
    for (int i = 0; i < 28; i++)
    {
      ...
      {
        drawBarGraph(barGraphMap[i][0], barGraphMap[i][1], 1);
      }
      if (pgm_read_byte_near(BGLEDStateFire1[IndexSegment]+ i) == 0)
      {
        ...
        {
          drawBarGraph(barGraphMap[i][0], barGraphMap[i][1], 0);
        }
  ...
  if ((currentMillis - lastUpdateBG) > IntervalBG)
  {
    ...
    else
    {
      for (int i = 0; i <= 27; i++)
      {
        if (i == IndexSegment )
        {
          drawBarGraph(barGraphMap[i + 1][0], barGraphMap[i + 1][1], 0);
        }
      }
    }
    if (IndexSegment == 27)
...
*/
    drawBarGraph(barGraphMap[IndexSegment+1 ][0], barGraphMap[IndexSegment+1 ][1], 0);
...
{
  if (_invertLogic) {
    bargraph.setPixel(row, col, !val); // Use inverted logic
  } else {
    bargraph.setPixel(row, col, val); // Use normal logic
  }
}

/*
   Draw function to allow simple bargraph wiring test
*/
void BGSequence::testLed(uint8_t led, uint8_t val) {
  // Use the internal map to find the correct row and column
  uint8_t row = barGraphMap[led][0];
  uint8_t col = barGraphMap[led][1];

  // Call the existing draw function
  drawBarGraph(row, col, val);

  // Push the change to the display
  bargraph.write();
}
